// Hand-maintained until you run `npm run db:types` (supabase gen types).
// ครอบคลุมตารางที่ BFF ใช้งานจริง
...
type Tbl<R, I = Partial<R>, U = Partial<R>> = { Row: R; Insert: I; Update: U };
...
export interface Database {
  public: {
    Tables: {
      ...
      audit_logs: Tbl<{ id: number; job_id: string | null; user_id: string | null; action: string; table_name: string; record_id: string | null; old_value: unknown; new_value: unknown; created_at: string }>;
    };
    Functions: Record<string, never>;
    Enums: {
      role_t: Role; channel_t: Channel; job_status_t: JobStatus; prod_status_t: ProdStatus;
      inst_status_t: InstStatus; issue_status_t: IssueStatus; payment_type_t: PaymentType;
    };
  };
}
