...
export const GET = withRoute(async (_req: Request, { params }: Params) => {
  ...
  if (!showFinance) {
    ...
    out.finance_entries = (out.finance_entries ?? []).filter((f: any) => !f.is_voided);
  }
  return ok(out);
});
...
const statusSchema = z.discriminatedUnion("status", [
  z.object({ status: z.literal("DEPOSITED"), deposit_amount: z.number().positive("ระบุยอดมัดจำ"), deposit_date: z.string() }),
  z.object({ status: z.literal("CANCELLED"), cancel_reason: z.string().min(1, "ระบุเหตุผลยกเลิก") }),
...
export const PATCH = withRoute(async (req: Request, { params }: Params) => {
  ...
  if ("status" in body) {
    ...
    const { data, error } = await ctx.supabase
      .from("jobs").update(payload).eq("id", params.id).select().single();
    if (error) throw new Error(error.message);
    await audit({ jobId: params.id, userId: ctx.user.id, action: "STATUS_CHANGED", table: "jobs", recordId: params.id, newValue: { status: payload.status } });
    return ok(data);
  }
