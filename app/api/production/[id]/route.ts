...
// READY → Installation, notes → Issue, status_updated_at: ทำใน DB trigger
...
export const PATCH = withRoute(async (req: Request, { params }: Params) => {
  ...
  if (body.status) await audit({ jobId: data.job_id, userId: ctx.user.id, action: "PRODUCTION_STATUS", table: "productions", recordId: params.id, newValue: { status: body.status } });
  return ok(data);
});
