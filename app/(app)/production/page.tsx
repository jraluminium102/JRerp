...
const COLS: ProdStatus[] = ["PENDING_MEASURE", "MEASURED", "QUEUED", "MANUFACTURING", "QC", "READY"];

export default function ProductionPage() {
  const { data, isLoading } = useQuery({ queryKey: ["jobs", "prod"], queryFn: () => api.get<Row[]>("/jobs?limit=100").then((r) => r.data) });
  const jobs = (data ?? []).filter((j) => j.productions?.length);

  return (
    <div className="p-4 sm:p-6 fade-in">
      <h1 className="text-xl sm:text-2xl font-bold text-white">Production</h1>
      <p className="text-sm mb-5" style={{ color: "var(--t-low)" }}>วัดจริง → ผลิต → QC → พร้อมติดตั้ง</p>
      {isLoading ? <Spinner /> : (
